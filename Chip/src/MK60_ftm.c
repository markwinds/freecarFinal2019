...
static void FTM_port_mux(FTMn_e ftmn, FTM_CHn_e ch) {
  ...
  case FTM1:
    ...
    switch (ch) {
    case FTM_CH0:
      if ((FTM1_CH0_PIN == PTA8) || (FTM1_CH0_PIN == PTA12) ||
          (FTM1_CH0_PIN == PTB0)) {
    ...
    case FTM_CH1:
      if ((FTM1_CH1_PIN == PTA9) || (FTM1_CH1_PIN == PTA13) ||
          (FTM1_CH1_PIN == PTB1)) {
 ...
 *  @since      v5.0
 *  @note
 * 同一个FTM，PWM频率是必须一样的，但占空比可不一样。共3个FTM，即可以输出3个不同频率PWM
 *  Sample usage:       ftm_pwm_init(FTM0, FTM_CH6,200, 10);    //初始化
 * FTM0_CH6 为 频率 200Hz 的PWM，占空比为 10/FTM0_PRECISON
 */
void ftm_pwm_init(FTMn_e ftmn, FTM_CHn_e ch, uint32 freq, uint32 duty) {
  ...
  /*       计算频率设置        */
  //  若 CPWMS = 1 ，即双边捕捉脉冲，则 PMW频率 =  bus频率 /2
  //  /(2^预分频因子)/模数 若 CPWMS = 0 ，即单边捕捉脉冲，则 PMW频率 =  bus频率
  //  /(2^预分频因子)/模数 EPWM的周期 ：MOD - CNTIN + 0x0001   (CNTIN 设为0)
  //  脉冲宽度：CnV - CNTIN

  //  模数 MOD < 0x10000
  //  预分频因子 PS  < 0x07
  //  预分频因子 PS 越小时，模数 mod 就越大，计数就越精准，PWM输出更为准确
  //  MOD  = clk_hz/(freq*(1 << PS)) < 0x10000  ==>  clk_hz/(freq*0x10000) <
  //  (1<< PS)  ==>  (clk_hz/(freq*0x10000) >> PS) < 1 即 (((clk_hz/0x10000 )/
  //  freq ) >> PS ) < 1
  ...
  ps = 0;
  while ((mod >> ps) >=
         1) // 等 (mod >> ps) < 1 才退出 while 循环 ，即求 PS 的最小值
  {
    ps++;
  }

  ASSERT(ps <= 0x07); // 断言， PS 最大为 0x07 ，超过此值，则
                      // PWM频率设置过低，或 Bus 频率过高

  mod = (clk_hz >> ps) / freq; // 求 MOD 的值

  ftm_mod[ftmn] = mod;

  switch (ftmn) // 初值 CNTIN 设为0 ，脉冲宽度：CnV - CNTIN ，即 CnV 就是
                // 脉冲宽度了。
  {
  ...
  /******************** 配置时钟和分频 ********************/
  FTM_SC_REG(FTMN[ftmn]) =
      (0
       //| FTM_SC_CPWMS_MASK         //0：上升沿计数模式 ，1：
       //跳变沿计数模式选择 （注释了表示 0）
       | FTM_SC_PS(ps) //分频因子，分频系数 = 2^PS
       | FTM_SC_CLKS(
             1) //时钟选择， 0：没选择时钟，禁用； 1：bus 时钟； 2：MCGFFCLK；
                //3：EXTCLK（ 由SIM_SOPT4 选择输入管脚 FTM_CLKINx）
  ...
  FTM_CNT_REG(FTMN[ftmn]) =
      0; //计数器。只有低16位可用（写任何值到此寄存器，都会加载 CNTIN 的值）
}
 ...
 *  @note
 * 同一个FTM，PWM频率是必须一样的，但占空比可不一样。共3个FTM，即可以输出3个不同频率PWM
 *  Sample usage:       ftm_pwm_duty(FTM0, FTM_CH6, 10);    //设置
 * FTM0_CH6占空比为 10/FTM0_PRECISON
 */
 ...
 *  @param      freq    频率（单位为Hz）
 *  @since      v5.0
 *  @note       修改PWM频率后，必须调用 ftm_pwm_duty
 * 重新配置占空比。同一个模块，PWM频率必须相同。 Sample usage:
 ...
 *  Sample usage:       ftm_input_init(FTM0, FTM_CH0, FTM_Rising,FTM_PS_2);
 * //设置 FTM0_CH0上升沿触发输入捕捉，二分频
 */
void ftm_input_init(FTMn_e ftmn, FTM_CHn_e ch, FTM_Input_cfg cfg, FTM_PS_e ps) {
  ...
  case FTM_Rising: //上升沿触发
    FTM_CnSC_REG(FTMN[ftmn], ch) |=
        (FTM_CnSC_ELSA_MASK | FTM_CnSC_CHIE_MASK); //置1
    FTM_CnSC_REG(FTMN[ftmn], ch) &=
        ~(FTM_CnSC_ELSB_MASK | FTM_CnSC_MSB_MASK | FTM_CnSC_MSA_MASK); //清0
    break;

  case FTM_Falling: //下降沿触发
    FTM_CnSC_REG(FTMN[ftmn], ch) |=
        (FTM_CnSC_ELSB_MASK | FTM_CnSC_CHIE_MASK); //置1
    FTM_CnSC_REG(FTMN[ftmn], ch) &=
        ~(FTM_CnSC_ELSA_MASK | FTM_CnSC_MSB_MASK | FTM_CnSC_MSA_MASK); //清0
    break;

  case FTM_Rising_or_Falling: //上升沿、下降沿都触发
    FTM_CnSC_REG(FTMN[ftmn], ch) |=
        (FTM_CnSC_ELSB_MASK | FTM_CnSC_ELSA_MASK | FTM_CnSC_CHIE_MASK); //置1
    FTM_CnSC_REG(FTMN[ftmn], ch) &=
        ~(FTM_CnSC_MSB_MASK | FTM_CnSC_MSA_MASK); //清0
    break;
  }

  FTM_SC_REG(FTMN[ftmn]) = (0 | FTM_SC_CLKS(0x1) //选择 bus 时钟
 ...
 *  @param      FTM_CHn_e       通道号（CH0~CH7）
 *  @param      FTM_Input_cfg   输入捕捉触发条件配置
 *  @return     获取捕捉事件发生时的计数器值
 * (根据两次捕捉之间的差值可判断周期频率)
 *  @since      v5.0
 *  Sample usage:       uint16 data = ftm_input_get (FTM0, FTM_CH0) ;   //
 * 获取FTM0_CH0输入捕捉事件触发时的计数值
 */
 ...
 *  Sample usage:       set_vector_handler(FTM0_IRQn , FTM1_Input_test_handler);
 * //把 FTM1_Input_test_handler 函数添加到中断向量表，不需要我们手动调用
 */
void FTM1_Input_test_handler(void) {
  uint8 s = FTM1_STATUS; //读取捕捉和比较状态  All CHnF bits can be checked
                         //using only one read of STATUS.
  uint8 CHn;
...
void ftm_quad_init(FTMn_e ftmn) {
  ...
  FTM_MODE_REG(FTMN[ftmn]) |= (0 | FTM_MODE_WPDIS_MASK //写保护禁止
                               //| FTM_MODE_FTMEN_MASK   //使能 FTM
  );
  FTM_QDCTRL_REG(FTMN[ftmn]) |= (0 | FTM_QDCTRL_QUADMODE_MASK);
  FTM_CNTIN_REG(FTMN[ftmn]) = 0;
  FTM_MOD_REG(FTMN[ftmn]) = FTM_MOD_MOD_MASK;
  FTM_QDCTRL_REG(FTMN[ftmn]) |= (0 | FTM_QDCTRL_QUADEN_MASK);
 ...
 *  @param      FTMn_e    模块号（ FTM1、  FTM2）
 *  @since      v5.0
 *  Sample usage:       int16 count = ftm_quad_get(FTM1);    //获取  FTM1 交解码
 * 的脉冲数
 */
