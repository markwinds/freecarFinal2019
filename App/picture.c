...
void writePictureToFlash()
{
    ...
    for (int i = 0; i < write_num; i++)           //写入图片数据
    {
        ...
        data = ((uint64)imgbuff[8 * i] << 56) + ((uint64)imgbuff[8 * i + 1] << 48) + ((uint64)imgbuff[8 * i + 2] << 40) + ((uint64)imgbuff[8 * i + 3] << 32) + ((uint64)imgbuff[8 * i + 4] << 24) + ((uint64)imgbuff[8 * i + 5] << 16) + ((uint64)imgbuff[8 * i + 6] << 8) + ((uint64)imgbuff[8 * i + 7]);
...
void writeBuffToFlash(int location_id)
{
    ...
    for (int i = 0; i < write_num; i++)           //写入图片数据
    {
        FLASH_WRITE_TYPE data;
#if defined(MK60DZ10)
        data = ((uint32)flash_imgbuff[4 * i] << 24) + ((uint32)flash_imgbuff[4 * i + 1] << 16) + ((uint32)flash_imgbuff[4 * i + 2] << 8) + ((uint32)flash_imgbuff[4 * i + 3]);
        flash_write(sector_id, sector_offset * 4, data);
#elif defined(MK60F15)
        data = ((uint64)flash_imgbuff[8 * i] << 56) + ((uint64)flash_imgbuff[8 * i + 1] << 48) + ((uint64)flash_imgbuff[8 * i + 2] << 40) + ((uint64)flash_imgbuff[8 * i + 3] << 32) + ((uint64)flash_imgbuff[8 * i + 4] << 24) + ((uint64)flash_imgbuff[8 * i + 5] << 16) + ((uint64)flash_imgbuff[8 * i + 6] << 8) + ((uint64)flash_imgbuff[8 * i + 7]);
        flash_write(sector_id, sector_offset * 8, data);
#endif
        sector_offset++;
    }
